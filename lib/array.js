...
internals.checkItems = function (items, wasArray, state, options) {
    ...
    for (let i = 0; i < il; ++i) {
        ...
        const localState = { key, path, parent: items, reference: state.reference, label: this._label };
        const localError = { key: state.key, path: localState.path, label: localState.label };
        let res;

        // Sparse

        if (!this._flags.sparse && item === undefined) {
            errors.push(this.createError('array.sparse', null, localError, options));
        ...
        for (let j = 0; j < this._inner.exclusions.length; ++j) {
            ...
            if (!res.errors) {
                errors.push(this.createError(wasArray ? 'array.excludes' : 'array.excludesSingle', { pos: i, value: item }, localError, options));
                errored = true;
        ...
        if (this._inner.ordereds.length) {
            if (ordereds.length > 0) {
                ...
                if (!res.errors) {
                    ...
                    errors.push(this.createError('array.ordered', { pos: i, reason: res.errors, value: item }, localError, options));
                    if (options.abortEarly) {
                        return errors;
                    }
                }
                ...
                errors.push(this.createError('array.orderedLength', { pos: i, limit: this._inner.ordereds.length }, localError, options));
                if (options.abortEarly) {
                    return errors;
                }
                continue;
        ...
        for (let j = 0; j < jl; ++j) {
            ...
            if (jl === 1) {
                ...
                errors.push(this.createError(wasArray ? 'array.includesOne' : 'array.includesOneSingle', { pos: i, reason: res.errors, value: item }, localError, options));
                errored = true;
        ...
        if (this._inner.inclusions.length && !isValid) {
            ...
            errors.push(this.createError(wasArray ? 'array.includes' : 'array.includesSingle', { pos: i, value: item }, localError, options));
...
internals.fillMissedErrors = function (errors, requireds, state, options) {
    ...
    if (knownMisses.length) {
        if (unknownMisses) {
            errors.push(this.createError('array.includesRequiredBoth', { knownMisses: knownMisses, unknownMisses: unknownMisses }, state, options));
        }
        else {
            errors.push(this.createError('array.includesRequiredKnowns', { knownMisses: knownMisses }, state, options));
        }
    }
    else {
        errors.push(this.createError('array.includesRequiredUnknowns', { unknownMisses: unknownMisses }, state, options));
    }
};
